...
| `/api/partner` | `Origin` header | Actually tries to restrict access (unlike `/api/account`), but via a naive `origin.startswith("http://" + host)` string check | CORS Misconfiguration, subdomain-prefix bypass — a plain unrelated Origin is correctly rejected, `http://<host>.evil.example` is not |
