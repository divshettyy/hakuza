...
def cmd_autopilot(args, console: Console) -> None:
    ...
    Unattended end-to-end pipeline for a fresh engagement:
      ...
      7. report     — final markdown + HTML report (also needs ANTHROPIC_API_KEY —
                       cmd_report has no non-AI mode, so it's skipped alongside
                       analyze/chain when no key is available)
    ...
    if have_ai:
        ...
        # cmd_report calls get_client() (not get_client_or_none()) and will block
        # on an interactive API-key prompt if no key is set — unsafe for an
        # unattended run, so it's only included here once AI is confirmed available.
        phases.append(("report", cmd_report, argparse.Namespace(html=True, output=None, client=None)))
    else:
        console.print(
            "\n[dim]Skipping AI-dependent phases (analyze, chain, report) — "
            "set ANTHROPIC_API_KEY and re-run, or use 'nexus findings --export json' "
            "for a non-AI findings export.[/dim]"
        )
